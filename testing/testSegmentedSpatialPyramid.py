...
#                time_sift_start = time.time()
                kp, des = detDes.featureDetectDesSIFT(roiImage)
#                time_sift_end = time.time()
#                print 'time in sift ' + str(time_sift_end - time_sift_start )
                if np.size(kp)>0:
#                   time_dist_compute_start = time.time()
                   histPoints = sp.buildHistogramForEachImageAtDifferentLevels(roiImage.shape[1], roiImage.shape[0], kp, des, 1,codeBookCenters)
