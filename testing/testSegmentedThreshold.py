...
def processAndClassify(frame,codeBookCenters, svm):
    ...
    ##Bilateral Filtering##
    #meanShiftResult = cv2.bilateralFilter(grayScaleInput,11,5,5)
    meanShiftResult = cv2.GaussianBlur(np.uint8(grayScaleInput),(11,11),0)
    #cv2.imwrite('meanShiftResult.png',meanShiftResult)
    ...
    #cv2.imwrite('meanShiftAdapResult.png',meanShiftAdapResult)
...
#    opening = cv2.dilate(meanShiftAdapResult,kernel,iterations=1)
    ...
    for cnt in contours:
        ...
        if cv2.contourArea(cnt)>500 and cv2.contourArea(cnt)<40000:
 #           if ...
...
#                print 'time in sift ' + str(time_sift_end - time_sift_start)
                if np.size(kp)>0: #and np.size(roiImage)< 40000 and np.shape(roiImage)[1] < 150:
#                   kpX = []
#                   kpY = []
...
#                       kpY.append(key.pt[1])
#
#                   medianValueX = np.median(kpX)
#                   print np.max(medianValueX)
#                   medianValueY = np.median(kpY)
#                   median = (medianValueX, medianValueY)
##
...
#                   time_randomForest_start = time.time()
                   ...
                   #cv2.imwrite(str(timeComp) +'Contour' + str(count) + 'Predicted' + str(prediction) + 'centroid' + str(centroid) + '.png',roiImage)
