...
HIDDEN_LAYER_TYPE = {"Common":"Common", "LSTM":"LSTM"}
...
def createNetworkState(parametersChannels, parametersHiddenLayer, parametersCrossChannels, parametersClassifier, channels, crossChannels):
    ...
    networkState.append(parametersClassifier.params)     
...
def loadData(log, experimentDirectory, channels, loadImagesStrategy, nFoldPositionsSet, nFoldIndex, channelsRepositories, classesLabel):    
    ...
    if len (channels[0][0][3]) == 2:
        ...
        testSet, audioSet = createSetFromPositions(setPositions, channels, channelsRepositories[0], loadImagesStrategy)
        validSet = testSet              
        ...
        trainSet, audioSets = createSetFromPositions(setPositions, channels, channelsRepositories[0], loadImagesStrategy) 
        
                                                
    else:                          
        log.printMessage(("Reading Train set"))        
        trainSet, audioSet = createSetFromPositions(trainPositions, channels, channelsRepositories[0], loadImagesStrategy)              
        log.printMessage(("Reading Valid set"))        
        validSet, audioSet = createSetFromPositions(validationPositions, channels, channelsRepositories[1], loadImagesStrategy)              
        log.printMessage(("Reading Test set"))        
        testSet, audioSet = createSetFromPositions(testPositions, channels, channelsRepositories[2], loadImagesStrategy)             
        nFoldPositionsSet = []
...
#            print "Shape data:", numpy.array(data).shape
            #cv2.imwrite("/informatik2/wtm/home/barros/Workspace/MCCNN2/temp3.png", data)
            #raw_input("here")
            
        elif dataModality == DATA_MODALITY["Face"]:
            data = cv2.imread(data)            
            data = ImageProcessingUtil.detectFace(data)    
            data = ImageProcessingUtil.resize(data,imageSize)
...
def createSetFromPositions(positionsSet, channels, channelRepositories, loadImagesStrategy):
