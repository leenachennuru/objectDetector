...
with finite (or infinite) propagation speed:
  ...
  âˆ‚V(x,t)                     âŒ                        |x-y|
Ï„ ------- = I(x,t) - V(x,t) + âŽ®  K(|x-y|) S( V(y, t - -----) ) dÂ²y
    âˆ‚t                        âŒ¡Î©                        c

where # V(x,t) is the potential of a neural population at position x and time t
      # Î© is the domain of integration of size lxl (mmÂ²)
      # K(x) is a neighborhood function from [0,âˆš2l] -> â„
      # S(x) is the firing rate of a single neuron from  â„âº -> â„
      # c is the velocity of an action potential (mm/s)
      # Ï„ is the temporal decay of the synapse
...
def peel(Z, center=None, r=8):
    ...
    :Returns:
        `out` : [numpy.ndarray,...]
            List of n Z-onion rings with n â‰¥ 1
    '''
    if r <= 0 :
...
def gaussian(x, sigma=1.0):
    ''' Gaussian function of the form exp(-xÂ²/ÏƒÂ²)/(Ï€.ÏƒÂ²) '''
    return 1.0/(sigma**2*np.pi)*np.exp(-x**2/(sigma**2))

def g(x, sigma=1.0):
    ''' Gaussian function of the form exp(-xÂ²/2ÏƒÂ²)) '''
...
if __name__ == '__main__':
    ...
    print '---------------------'
    print 'Size of the field        : %.1fmmÃ—%.1fmm' % (l,l)
    print 'Action potential velocity: %.1fmm/s' % c
    print 'Tau                      : %.2f' % tau
    print 'Space discretisation     : %dÃ—%d' % (n,n)
